...
namespace PATSOrder.Class
{
    ...
    public class Tick : IDisposable
    {
        ...
        public Tick(int tickNumber, bool extend = false, bool ascending = true, float fontsize = 8)
        {
            ...
            Center.Instance.AddObserver(_BeforePATSReset, Util.PATS_BEFORERESET);
            Center.Instance.AddObserver(_AfterPATSReset, Util.PATS_AFTERRESET);
            _AfterPATSReset(null, null);
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing)
                {
                    ...
                    _BeforePATSReset("", new Msg(null, null));
                    Center.Instance.RemoveObserver(_BeforePATSReset, Util.PATS_BEFORERESET);
                    Center.Instance.RemoveObserver(_AfterPATSReset, Util.PATS_AFTERRESET);
                }
        ...
        #region Delegate
        private void _BeforePATSReset(string MsgName, Msg Msg)
        {
            Util.PATS.OnPriceUpdate -= PATS_OnPriceUpdate;
            if (ProductInfo != null) { Util.PATS.Unsubscribe(ProductInfo.Key); }
        }
        private void _AfterPATSReset(string MsgName, Msg Msg)
        {
            Util.PATS.OnPriceUpdate += PATS_OnPriceUpdate;
