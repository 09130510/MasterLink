...
static NuSocketNode_t *ExpandAvailableNodeAndGetOne(NuSocket_t *Socket, int ExpandCnt)
{
    ...
    while(ExpandCnt --)
    {
        ...
        NuStrNew2(&(pNode->TypeNode.TargetAddr), 16, NULL);
        NuStrNew2(&(pNode->TypeNode.LocalAddr), 16, NULL);
...
static void AddToEpoll(NuSocketNode_t *Node)
{
    struct epoll_event  Event;
...
int NuSocketRecvInTime(NuSocketNode_t *Node, void *Msg, size_t MsgLen, int Sec)
{
    NuSocketTypeNode_t  *pTypeNode = &(Node->TypeNode);
    int                 RecvLen = 0;
    int                 Cnt = 0;
    ...
    while(MsgLen > 0)
    {
        switch(RecvLen = Node->Type->Recv(pTypeNode, pRecv, MsgLen))
        {
        case NuSocketTypeCBError:
        case NuSocketTypeCBDone:
            MsgLen = 0;
...
{
    return NuStrGet(Node->TypeNode.TargetAddr);
}
