...
static int _ReMMap(NuMMap_t *pmmap)
{
	...
	if (pmmap->pMPStrm->addr == MAP_FAILED)
		return NU_MMAPERR;
	pmmap->pMPStrm->addr += (pmmap->data_len - lalign);
...
static int _ReMMap2(NuMMap_t *pmmap)
{
	...
	pmmap->pMPStrm->addr += pmmap->data_len;
	pmmap->pMPStrm->fsz = nfsz;
...
/* functions */
/* ====================================================================== */
int NuMMapNew(NuMMap_t **pmmap, char *pMFile, char *mode, size_t len, int prot, int flags)
{
