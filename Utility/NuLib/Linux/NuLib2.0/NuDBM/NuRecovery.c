...
{
    NuEventHdlr_t           *Ev[NuDBMTableEventTypeNo];
    NuFileStream_t          *Log;
    NuTable_t               *Table;
} NuRecoveryEvSet_t;
...
static void NuRecoveryInsert(void *Rec, void *Argu)
{
...
static void NuRecoveryUpdateInsert(void *Rec, void *Argu)
{
    NuRecordSet_t   *pRS = (NuRecordSet_t *)Rec;
    NuTable_t       *pTable = pRS->pTable;
...
static void NuRecoveryUpdateDelete(void *Rec, void *Argu)
{
    NuRecordSet_t   *pRS = (NuRecordSet_t *)Rec;
    NuTable_t       *pTable = pRS->pTable;
...
static void NuRecoveryDelete(void *Rec, void *Argu)
{
    NuRecordSet_t   *pRS = (NuRecordSet_t *)Rec;
    NuTable_t       *pTable = pRS->pTable;
...
void NuDBMRecover(char *RecoverLogPath, char *TableName)
{
    ...
    if(NuFStreamOpen(&pFS, RecoverLogPath, "r") > 0)
    {
        while((LogLen = NuFStreamReadLineByStr(pFS, pStr)) > 0)
        {
            ...
            *pPipe = '\0';
            ...
            case NuRecovery_DeleteSign:
                ...
                NuRSSetTran(pRS);
                NuRSSelectByKey(pRS, Table->PKey, pPipe, Cnt);
                NuRSDelete(pRS);
                NuRSCommit(pRS);
...
static void _NuRecoveryFormatDelete(NuTranLog_t *pTranLog, NuStr_t *pStr, size_t Len)
{
    ...
    NuCStrReplaceRangeChr(pData, '\0', '|', 0, Len);
    ...
    while(pRecovery->ThreadAlive || !NuHQIsEmpty(pRecovery->Q))
    {
        if(NuHQDequeue(pRecovery->Q, &ptr, &LogLen) == NU_OK)
        {
            ...
            default:
                ...
                NuCStrReplaceRangeChr(pTranLog->Data, NuRecovery_Delimiter, NuRecovery_ReplaceDelimiterTo, 0, LogLen);
...
static int NuRecoveryStart(size_t LogSize, char *Path)
{
    ...
    RC = NuHQNew(&(pRecovery->Q), NuHQ_DEFAULT_NUM);
    NUCHKRC(RC, EXIT);
    ...
    while(VecIt != base_vector_it_end(pVec))
    {
        ...
        NuStrPrintf(pStr, "%s%c%s.tlog", Path, NUFILE_SEPARATOR, pTable->Name);
        ...
        {
            if(NuCreateRecursiveDir(Path) < 0)
            {
                return NU_FAIL;
            }
