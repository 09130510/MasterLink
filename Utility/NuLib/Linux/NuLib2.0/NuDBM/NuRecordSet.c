...
static int TrySetTran(NuRecordSet_t *pRec, NuIndex_t *pIndex)
{
    ...
    NuHashIterator_t    *pIt = &(pRec->HashIt);
...
static void NuRSClean(NuRecordSet_t *pRec, int Success)
{
    ...
    base_vector_it_set(pRec->VecIt, pVec);

    while(pRec->VecIt != base_vector_it_end(pVec))
    {
        ...
        { /* Update, release the row lock. */
            pRow->TranID = NuRowTranID_NORMAL;
        }
        else
        { /* Delete, or Insert failed. */
            base_vector_it_set(VecIt, pVec = pTable->Index);
...
{
    return base_list_items_cnt(&((NuHashItGetGroup(&(pRec->HashIt)))->List));
}
...
{
    NuRow_t *pRow = (NuRow_t *)((pRec->TranID < 0) ? NuHashItGetValue(&(pRec->HashIt)) : (*(pRec->VecIt)));
