...
static void DBMTimerEvent(void *Argu)
{
    ...
    while(TableVecIt != base_vector_it_end(TableVec))
    {
        ...
        while(IndexVecIt != base_vector_it_end(IndexVec))
        {
            ...
            {
                NuHashRmvItemComplete(pIndex->IndexMap, ptr);
            }

            while(base_vector_pop(pIndexTrash->GroupVec[Tmp], &ptr) >= 0)
            {
                NuHashRmvGroupComplete(pIndex->IndexMap, ptr);
            }
...
static void NuTableFree(NuTable_t *pTable)
{
    ...
    if(pTable != NULL)
    {
        ...
        {
            free(base_vector_get_by_index(pTable->Schema, 0));
            base_vector_free(pTable->Schema);
        }

        for(Cnt = 0; Cnt < NuDBMTableEventTypeNo; ++ Cnt)
        {
            NuCBMgrDelTrigger(pTable->Event[Cnt]);
        }
...
NuTable_t *NuDBMAddTable(char *TableName, int DefaultRowNum, int ColNum, ...)
{
    ...
    {
        pTable->Event[RC] = NuCBMgrAddTrigger();
    }
...
NuEventHdlr_t *NuTableRegisterEvent(NuTable_t *Table, NuDBM_TableEventType Type, NuCBFn Fn, void *Argu)
{
...
void NuTableUnRegisterEvent(NuEventHdlr_t *EventHdlr)
{
    NuCBMgrUnRegisterEvent(EventHdlr);
    return;
}
