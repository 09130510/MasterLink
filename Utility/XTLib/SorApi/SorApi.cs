/**
 * 不支援 .NET 2 x64: http://connect.microsoft.com/VisualStudio/feedbackdetail/view/488941/p-invoke-memory-faults-when-marshaling-byref-ansibstr-on-x64
 * 請使用 .NET 4 x64
 */
using System;
...
namespace SorApi
{
    ...
    public class SorClient : IDisposable
    {
        ...
        public void Connect(string connParam, string sysid, string user, string pass, string APVer)
        {
            object[] attribute = this.GetType().Assembly.GetCustomAttributes(typeof(AssemblyTitleAttribute), false);
            AssemblyTitleAttribute title = (AssemblyTitleAttribute)(attribute[0]);

            CSorClient_Connect(ref Impl_, connParam, title.Title, APVer, sysid, user, pass);
        }
