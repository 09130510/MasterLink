...
namespace SorApi
{
    ...
    public class DigSgnHandler : IDisposable
    {
        ...
        CAErrCode ErrCode_ = CAErrCode.ERR_DLLNotFound;
        ...
        public DigSgnHandler(string dllname, string certConfig, int sgnact)
        {
            ...
            ErrCode_ = CAErrCode.ERR_DLLUnknown;
    ...
    public class SorAcc
    {
        ...
        /// </summary>
        public SorAcc(SorApi.SorMktFlags mkt, string acno, string name)
        {
        ...
        /// 建構.
        /// </summary>
        public SorAcc(SorApi.SorMktFlags mkt, string brkNo, string ivacNo, string subacNo, string name)
        {
        ...
        public CAErrCode CAErrCode
        {
            get { return DigSgnHandler_ == null ? CAErrCode.ERR_Success : DigSgnHandler_.CAErrCode; }
        }
    }
    /// <summary>
    /// 交易帳號列表.
    /// </summary>
    public class SorAccs
    {
        List<SorAcc> List_ = new List<SorAcc>();
        SortedList<string, SorAcc> Sorted_ = new SortedList<string, SorAcc>();
        /// <summary>
        /// 增加一個帳號.
        /// </summary>
        public bool Add(SorAcc acc, string caDLLName, string cert, int sgnact)
        {
        ...
        /// </summary>
        public bool TryGetValue(string acno, out SorAcc ac)
        {
        ...
        /// 取得帳號列表.
        /// </summary>
        public List<SorAcc> Values { get { return List_; } }
        /// <summary>
        ///  取得帳號筆數.
        /// </summary>
        ...
        /// 從 SorTable 取得可用帳號列表.
        /// </summary>
        public void SorTableParser(SorApi.SorTable table)
        {
            if (table.IsInvalid)
                return;
            ...
            {  // acno 格式: brkno-ivacno-subacno.
                ...
                for (L = 0; L < rcount; ++L)
                {
                    ...
                    Add(new SorAcc((SorApi.SorMktFlags)imkt, acno, name));
                }
            }
            else
            {  // BHNO-ACNO-SUBA 格式
                ...
                for (L = 0; L < rcount; ++L)
                {
                    ...
                    int imkt;
                    int.TryParse(mkt, out imkt);
                    Add(new SorAcc((SorApi.SorMktFlags)imkt, bhno, ivac, suba, name));
                }
            }
        }
        /// <summary>
        ...
        public void SorTableParser(SorApi.SorTable table, string caDLLName, int sgnact)
        {
            ...
            {  // acno 格式: brkno-ivacno-subacno.
                ...
                for (L = 0; L < rcount; ++L)
                {
                    ...
                    int imkt;
                    int.TryParse(mkt, out imkt);
                    acc = new SorAcc((SorApi.SorMktFlags)imkt, acno, name);
            ...
            {  // BHNO-ACNO-SUBA 格式
                ...
                for (L = 0; L < rcount; ++L)
                {
                    ...
                    acc = new SorAcc((SorApi.SorMktFlags)imkt, bhno, ivac, suba, name);
                    Add(acc, caDLLName, table.RecordField(L, fldCert), sgnact);
