...
namespace SourceCell
{
	...
	public class ButtonCell : CellBase
	{
		...
		/// </summary>
		public static Color ButtonBackColor = Color.FromName("Control");
		/// <summary>
		...
		/// Cell
		/// </summary>
		private SourceGrid.Cells.Button field;
		/// <summary>
		/// Value
		/// </summary>
		...
		/// </summary>
		public override SourceGrid.Cells.ICell Cell { get { return field; } protected set { } }
		/// <summary>
		/// Cell
		/// </summary>
		public SourceGrid.Cells.Button Field
		{
			get { return field; }
			private set { }
		}
		...
		public new FontName FontName
		{
			...
			set
			{
				...
				if (field != null)
				{
					SetFontType((SourceGrid.Cells.Views.Button)field.View, fontname.ToString(), fontsize, fontstyle);
				}
			}
		}
		/// <summary>
		...
		public bool HasBorder
		{
			...
			set
			{
				...
				if (field != null)
				{
					SetBorder((SourceGrid.Cells.Views.Button)field.View, hasborder);
				}
			}
		}
		/// <summary>
		...
		public override bool Enable
		{
			...
			set
			{
				...
				if (field != null)
				{					
					//x field.Editor.EnableEdit = Enable;
					((SourceGrid.Cells.Views.Button)field.View).Background.Style = Enable ? DevAge.Drawing.ButtonStyle.Normal : DevAge.Drawing.ButtonStyle.Disabled;
				}
			}
		}
		/// <summary>
		...
		public override Color DefaultBackColor
		{
			get
			{
				if (dbackcolor == new Color()) { dbackcolor = ButtonBackColor; }
				return dbackcolor;
			}
			set
			{
		...
		public override Color DefaultFontColor
		{
			get
			{
				if (dfontcolor == new Color()) { dfontcolor = ButtonFontColor; }
				return dfontcolor;
			}
			set
			{
		...
		public override void SetBackColor(object _Color)
		{
			...
			//x if (field != null)
			//x {
			//x     if (field.Grid != null && field.Grid.InvokeRequired)
			//x     {

			//x         SetColor d = new SetColor(SetBackColor);
			//x         field.Grid.Invoke(d, new object[] { _Color });
			//x     }
			//x     else
			//x     {
			//x         ((SourceGrid.Cells.Views.Button)field.View).BackColor.Background = null;
			//x       DevAge.Drawing.VisualElements.Button bg = new DevAge.Drawing.VisualElements.Button();
			//x         bg.Border = DevAge.Drawing.RectangleBorder.CreateInsetBorder(1, BorderShadowColor, BorderLightColor);

			//x         if (_Color != null)
			//x         {
			//x             bg.BackColor = (Color)_Color;
			//x         }
			//x         else
			//x         {
			//x             //還原顏色                
			//x             bg.BackColor = DefaultBackColor;
			//x         }
			//x         ((SourceGrid.Cells.Views.ColumnHeader)field.View).Background = bg;

			//x         if (field.Grid != null)
			//x         {
			//x             SourceGrid.Range r = new SourceGrid.Range(field.Row.Index, field.Column.Index, field.Row.Index, field.Column.Index);
			//x             field.Grid.InvalidateRange(r);
			//x         }
			//x     }
			//x }
		}
		...
		{
			SourceGrid.Cells.Views.Button v = new SourceGrid.Cells.Views.Button();
			
			v.TextAlignment = textAlignment;
			v.ForeColor = DefaultFontColor;			
			//x v.BackColor = DefaultBackColor;
			SetFontType(v, fontname.ToString(), fontsize, fontstyle);
			SetBorder(v, hasborder);
			v.WordWrap = WordWrap;
			
			field = new SourceGrid.Cells.Button(fvalue);
			field.View = v;
