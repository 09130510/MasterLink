...
namespace Util.Extension.ZMQ
{
    public class Sub : BaseZMQ
    {
        ...
        //public delegate void OnSUBReceivedDelegate(string Head, byte[] Msg);
        ...
        public void Start()
        {
            ...
            if (m_Thread == null)
            {
                m_Thread = new Thread(new ThreadStart(() =>
                {
                    while (true)
                    {
                        //ZmqMessage msg = m_Subscriber.ReceiveMessage();
                        ...
                        if (OnSUBReceived != null && msg.FrameCount > 1)
                        {
                            ...
                            //OnSUBReceived(Encoding.ASCII.GetString(msg[0]),msg[1]);
                        }
                    }
                }));
            }
